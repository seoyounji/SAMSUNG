...
public class prob15685 {
	...
	private static void fill(Node node) { 
		...
		list.add(node.dir); int x = node.x + dx[node.dir]; 
		int y = node.y + dy[node.dir]; 
		setRange(x, y); int ndir; 
		while (gen-- > 0) { 
			for (int i = list.size() - 1; i >= 0; i--) { 
				ndir = (list.get(i) + 1) % 4; 
				x = x + dx[ndir]; 
				y = y + dy[ndir]; setRange(x, y); 
				map[x][y] = true; list.add(ndir); 
			} 
		} 
	} 
	...
	public static void main(String[] args) throws Exception { 
		...
		for (int i = 0; i < n; i++) { 
			...
			setRange(y, x); list[i] = new Node(y, x, dir, gen); 
		} 
		for (int i = 0; i < n; i++) { 
			fill(list[i]); 
		} 
